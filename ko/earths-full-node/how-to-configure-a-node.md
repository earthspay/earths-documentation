...
The configuration system of Earths Node uses HOCON format. HOCON stands for Human-Optimized Config Object Notation. The complete description of HOCON could be found in the [Official HOCON documentation](https://github.com/typesafehub/config/blob/master/HOCON.md). The advantages of HOCON are simple syntax and ability to use comments.

## Complete Earths Node configuration file 

| Note | This document describes configuration file format of Earths version 0.9.x |
| :--- | :--- |


Below you can find a complete Earths Node configuration file. This is the default configuration shipped with the application. It is possible to overwrite any parameters by providing an additional configuration file. You can pass an additional configuration file by providing the path to it as the first command line parameter then starting Earths Node application.

```js
# Earths node settings
earths {
  # Node base directory
  directory = ${user.home}"/earths"

  # Node data directory (LevelDB)
  data-directory = ${earths.directory}"/data"
  ...
  network {
    ...
    file = ${earths.directory}"/peers.dat"
    ...
    known-peers = ["18.231.27.49:7773", "52.210.14.4:7773"]
  ...
  wallet {
    # Path to wallet file
    file = ${earths.directory}"/wallet/wallet.dat"
  ...
  matcher {
    ...
    matcher-directory = ${earths.directory}"/matcher"
    data-directory = ${earths.matcher.matcher-directory}"/data"
    journal-directory = ${earths.matcher.matcher-directory}"/journal"
    snapshots-directory = ${earths.matcher.matcher-directory}"/snapshots"

    # LevelDB's internal cache size
    leveldb-cache-size = ${earths.leveldb-cache-size}
...
## Earths configuration section 

Root configuration section `earths` holds essential application parameters and other configuration subsections.
...
| Note | If you want to change earths directory in Ubuntu packages you should change it using `-J-Dearths.directory=path` in `/etc/earths/application.ini` and `/lib/systemd/system/earths.service`. You can override any JVM start parameter in `earths.service`, it has priority. |
| :--- | :--- |
...
Use `file` parameter to set the location of peers database. In this database node stores lists of known and blacklisted peers. By default, the path is resolved with regard to base `directory` from `earths` section.
...
Using parameter `bind-address` you can set the IP address of local network interface on which Earths Node will accept incoming connections. By default, node binds to `0.0.0.0` that means that it will listen on all available network adapters.

Use `port` parameter to set the network port number to which other Earths nodes will connect. Check that the port is reachable from outside otherwise, your node will connect to P2P network only using outgoing connections. If this the port is taken by other application, your node won’t start.
...
In `upnp` section you can set the UPnP settings. Actually, those settings are useful only if you ran your Earths node on the home network where the node could ask your router to establish a tunnel. By default, this functionality is disabled. Use`enable`parameter of`upnp`to enable this functionality.
...
In `wallet` section you can configure wallet built in Earths node.
...
Using `min-order-fee` parameter it’s possible to set the minimum required fee to process orders. For now, matcher accepts fee only in EARTHS. The fee should be given in minimal units \(EARTHLETS\).
...
To specify Earths as a side of a pair use special asset ID "EARTHS".
...
predefined-pairs = [
     {amountAsset = "EARTHS", priceAsset = "Fmg13HEHJHuZYbtJq8Da8wifJENq8uBxDuWoP9pVe2Qe"},
     {amountAsset = "EARTHS", priceAsset = "HyFJ3rrq5m7FxdkWtQXkZrDat1F7LjVVGfpSkUuEXQHj"},
     {amountAsset = "EARTHS", priceAsset = "2xnE3EdpqXtFgCP156qt1AbyjpqdZ5jGjWo3CwTawcux"},
     {amountAsset = "EARTHS", priceAsset = "6pmDivReTLikwYqQtJTv6dTcE59knriaodB3AK8T9cF8"},
...
In `micro-block-synchronizer` subsection you could tune various parameters of Earths-NG protocol.
