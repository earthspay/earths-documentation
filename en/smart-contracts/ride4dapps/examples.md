...
A dApp wallet is implemented where you can send payment as EARTHS and save them in the wallet via the callable _**deposit function**_ or you can take them back via the callable _**withdraw function**_.

_**As a first step**_, let's write a function which gets how much EARTHS did the **contract issuer** give to the contract. let's name this function **getBalance\(address\) :Int. **
...
Now we need to define a **deposit function**\(as Callable function\) where the user can send payment as EARTHS and save them in the wallet.
...
func deposit() = {
  ...
  let currentBalance = getBalance(caller) #how much EARTHS did the contract issuer give to the contract.
  ...
  if (payment.assetId != unit) then throw("This wallet cannot hold assets other than EARTHS")
  else {
...
Finally we need to write the **withdraw function** \(Callable function\) where the user can take EARTHS back.
...
In this Example, let's suppose that we need to implement a mechanism to ensure that funds in an account will be divided equally between two specific addresses Alice and Bob. To each of them we will send exactly \(\(balance - fee\) / 2\) EARTHS.
...
@Callable(i)
func divideAmount() = {
# calculate the amount of EARTHS that will be transferred to Alice and Bob
    let transferAmount = earthsBalance(this) / 2
