# Earths Console Commands

[Earths IDE](https://ide.earths.ga) has a Earths console feature which supports different commands:

**Note. **Using [Earths Transactions library](/earths-api-and-sdk/client-libraries/earths-transactions.md) you can easily create and sign transactions for Earths blockchain. It also allows you to multi-sign existing transactions or create them without signature at all.

# MultiSig Example

[_**Here**_](/technical-details/earths-contracts-language-description/earths-console-commands/examples.md) you can check the MultiSig smart contract example using Earths Console.
 ...
 * @param {string} assetId - Asset Id to transfer, default: 'EARTHS'.
 * @param {string} attachment - Attachment to transfer, default: ''.
 * @param {number} feeAssetId - Asset Id to pay fee with, default: 'EARTHS'.
 * @param {number} fee - Transaction fee, default: 100000.
 * @param {number} timestamp - Transaction timestamp, default: Date.now().
 * @param {number} version - Transaction version, default: 1.
 * @param {string} seed - Seed to sign transaction, default: env.SEED.
...
declare function transfer(
  amount: number,
  recipient: string,
  assetId: string = 'EARTHS',
  attachment: string = '',
  feeAssetId: string = 'EARTHS',
  fee: number = 100000,
  timestamp: number = Date.now(),
  version: number = 1,
  seed: string = env.SEED
...
```js
const aliasTx = alias({alias: 'earthspay'})
broadcast(aliasTx)
```
 ...
 * @param {string} assetId - Asset Id to transfer, in case you want to transfer EARTHS use default, default: ''.
 * @param {number} fee - Transaction fee, default: 200000.
 * @param {number} timestamp - Transaction timestamp, default: Date.now().
 * @param {number} version - Transaction version, default: 1.
 * @param {string} seed - Seed to sign transaction, default: env.SEED.
...
## Sends transaction to the Earths network using env.API\_BASE endpoint

* broadcast\(tx\)

```js
 /**
 * Sends transaction to the Earths network using env.API_BASE endpoint.
