...
2. If there is a counter-order that matches with the submitted order, then the order execution is performed. That means the counter-order is removed from order book and the matcher creates exchange transaction, signs it by matcher's private key and is sent to the Earths network for including in the blockchain.
...
The fix full transaction fee now is equal to **0.003 earths for one order**, whether buying or selling, regardless of the amount of the future deal. The exchange transaction contains two separate fields for Matcher's fee, which goes from buyer's order and seller's order. An order can be fully executed by some transaction, in this case, all matcher fee from it is included in that transaction.
...
There are 3 different orders \(Figure 2\): two buy orders and one sell. For each full order, a user has to pay exactly **0.003 earths** of a fee, and this fee will be written off as the order is executed. In our example:

* the Order1 is fully matched with a 70% part of Order3 by Transaction1 and matcher's fee for this transaction is equal to 0.003 + 0.0021 - 0.003 = 0.0021 earths since Matcher pay to miners transaction fee which is also equal to 0.003 Earths.
* The 50% of Order2 matches with 30% part of Order3 by Transaction2 and matcher's fee for this transaction is equal to 0.0009 + 0.0015 - 0.003 = -0.0006 earths.

Thus, the fee that the matcher gets from users for these transactions is**0.0021 - 0.0006  = 0.0015 earths**. And the fee that the matcher pays to miners is**0.006earths**.
...
| :--- | :--- | :--- | :--- |
| last month | 681.42336675 earths | 569.721 earths | 16.39% |
| all time | 3476.01418346 earths | 2824.771 earths | 18.74% |
...
* [EARTHS](../development-and-api/earths-node-rest-api/address.md#get-addressesbalanceaddress);
* [assets](../development-and-api/earths-node-rest-api/address.md#get-assetsbalanceaddress).

## spendings of asset in transactions in utx pool

The sum of all spending by the asset of unconfirmed transactions.

For example, if you are transferring `EARTHS` and sending a [data transaction](../earths-environment/earths-protocol/data-transaction.md)  
and these transactions haven't yet forged, `spendings_of_EARTHS_in_transactions_in_utx_pool` will be `amount_of_transferred_earths + transfer_fee + data_transaction_fee`.
...
Note, for `EARTHS` order's fees are included in reserved balance.

As you know, all orders in `DEX` requires `EARTHS` as the fee. Because of this, there is an exception for the rule we spoke above.  
If you buy `EARTHS` by other asset, reserved balance in earths in this order will be: `max(fee - amount_of_received_earths_in_this_order, 0)`.  
So, you can buy `EARTHS` for `BTC` even if you have no `EARTHS`.

# 5. Installing DEX 

* Download the Earths client from our official website,
  [www.earths.ga](http://www.earths.ga/)
  , or use the webwallet, available at
  [client.earths.ga](https://client.earths.ga/)
* [Deposit your bitcoins](/earths-client/transfers-and-gateways/bitcoin-transfers.md) or any [supported coins and tokens](/en/earths-client/wallet-management.md) into the wallet and [start trading using the Earths DEX](/earths-client/earths-dex.md).

# 6. Installing your Own Matcher 

* Users can install their own matcher by installing the Earths software and enabling the matching functionality.
