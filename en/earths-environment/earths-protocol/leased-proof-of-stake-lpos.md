...
Earths use so-called Proof-of-Stake consensus algorithm. Here we'll describe it and compare with widely used Proof-of-Work algorithm which used, for example, in Bitcoin.
...
* Minimum amount of EARTHS to generate is 1000 EARTHS
...
* Earths Implement the Proof-of-Stake method for a more greener and cheaper distributed form of consensus.
...
LPoS is an enhanced version of Proof-of-Stake. In a regular Proof-of-Stake system, each node that holds a certain amount of cryptocurrency is eligible to add the next block to the blockchain but in the LPoS system, on the Earths Platform, users can lease their balance to full nodes. WithLPoS, the user will have the ability to Lease EARTHS form the wallet to different contractors which can pay a percentage as a reward. The larger the amount that is leased to a full node, the higher the chances of that full node being selected to produce the next block. If that full node is selected to produce the next block, the leaser will then receive a percentage of the transaction fee that is collected by the full node.

In a Leased Proof-of-Stake environment, users can choose between running a full node or leasing their stake to a full node with receiving rewards. This system allows anyone to participate in the Earths network maintenance.

User can lease his EARTHS through leasing on any computer or mobile device that has an internet browser since Earths provides a lite client solution that does not require "Miners", that are leasing their balance to store the whole Blockchain or to have the wallet running, Figure 2.

![](/_assets/Webp.net-resizeimage-2.jpg)  
Figure 2, LPOS System

## 3.1 Benefits of leasing your EARTHS 

Safety \(your EARTHS never leave your wallet\):

* Node operators can use your mining power to generate blocks without the user even having to send their EARTHS at all.
...
* The balance of the node can be empty until there are enough people wishing to lease to it by reaching together the generating balance of 1000 EARTHS and create together a pool.
...
* These rewards mostly come in the form of EARTHS but also can come in the form of different tokens with the unique Earths feature where different tokens can be accepted as a fee.
...
For Step-by-Step guide to lease your earths, click [here](/en/earths-client/account-management/earths-leasing.md).

For the List of generators click [here](http://dev.pyearths.org/generators/).

For Earths Nodes click [here](https://earths.ga/leasing#nodes).
