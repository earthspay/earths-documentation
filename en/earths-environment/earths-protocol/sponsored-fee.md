...
The total **miner's fee in EARTHS for transactions** with a fee in sponsored assets can be compute by this formula:
```
...
But the total **block fee** for the block with sponsored transactions can be computed as the sum of transactions which have **only the fee in EARTHS**. For example, if we have the block with only sponsored transactions, a fee for this block will be equal to 0.
...
Sponsorship information for the asset present in [asset description](../earths-api-and-sdk/earths-node-rest-api/asset-transactions/public-functions.md#get-assetsdetailsassetid).
