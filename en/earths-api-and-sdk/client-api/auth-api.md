...
If you want to authorize a user in your service by means of his Earths account, here's the solution. In general, you should redirect the user to the official Earths Client \([https://client.earths.ga/](https://client.earths.ga/)\) with certain query parameters including some arbitrary data for him to sign.
...
1. You add the Earths Auth widget to your site.
2. A user stumbles upon your site and wants to log in using his Earths account.
3. He clicks the widget button and gets redirected to the official Earths Client, along with some random data from the widget.
...
[**Here**](https://demo.earths.ga) you can find the demo project which shows how to use Web auth API.

### Request

[Example](https://client.earths.ga#gateway/auth?r=https://example.com&n=Service%20Name&d=0123456789abc&i=/img/logo.png&success=/earthsAuth): `https://client.earths.ga#gateway/auth?r=https://example.com&n=Service%20Name&d=0123456789abc&i=/img/logo.png&success=/earthsAuth`.

Basic path is `https://client.earths.ga#gateway/auth`. Then the query parameters go.
...
You can use the [@earths/signature-generator](https://www.npmjs.com/package/@earths/signature-generator) npm package.
...
All strings is converted to `length bytes + value bytes` as in [Data Transactions](/earths-environment/earths-protocol/data-transaction.md)
...
#### Js example code
```javascript
const sg = require('@earths/signature-generator')
...
import pyearths.crypto as crypto
import base58
...
def signed_data(host, data):
    prefix = 'EarthsWalletAuthentication'
