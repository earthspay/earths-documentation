...
{% prettylink link="https://matcher.earths.ga/api-docs/index.html" %}MainNet Matcher API{% endprettylink %}

{% prettylink link="https://matcher.testnet.earthsnodes.earths.ga/api-docs/index.html" %}TestNet Matcher API{% endprettylink %}
...
The first asset in the pair is an asset with minimal bytes compared byte by byte starting from the first. EARTHS is always price asset in the pair (the second one), with the exception of assets in list: https://matcher.earths.ga/matcher/settings
...
| price | Long | Execution price of orders see[algorithm](/earths-api-and-sdk/dex-api/matcher.md). Price is determined for `Asset2` in `Asset1` \* 10^8 |
...
[How much](../earths-environment/earths-protocol/decentralized-cryptocurrency-exchange-dex.md#4-tradable-balance) you can spend in orders on the given pair.
...
So, in pair `zMFqXuoyrn5w17PFurTqxB7GsS71fp9dfk6XFwxbPCy/EARTHS` you can spend `1024460284 EARTHS` and `11 Bitcoin Cash`.
